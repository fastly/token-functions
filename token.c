...
int main(int argc, char** argv) {
    ...
    unsigned int md_len;
    unsigned char* digest = HMAC(EVP_sha256(), key, strlen(key), number, 8, NULL, &md_len);
    unsigned char* token  = encode64(digest, md_len);
    ...
    fprintf(stderr, "Your Token:   %sn", token);
    fprintf(stderr, "Fastly Token: %sn", response.ptr);
    fprintf(stderr, "Validation:   %sn", validation.ptr);
